...
class DataValidation: 
  ...
  def validate_numerical_columns(self) -> bool : 
    try :
      number_of_numeric_columns = len(self._schema_config['numerical_columns'])
      
      if number_of_numeric_columns > 0: 
        return True
      return False
  ...
  def initiate_data_validation(self) -> DataValidationArtifact :
    try:
      ...
      ## Reading the data from train test
      ...
      if not status:
        error_message = f"{error_message} There is no numerical columns present in the dataset"        
